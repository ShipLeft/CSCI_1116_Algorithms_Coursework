 ...
 * <h1>Exercise25_01</h1>
 *
 * <p>This class has been edited to include the height and breadth-first traversal of each tree.</p>
 *
 * <p>Created: 01/05/22</p>
 *
 * <h3>Edited_by Rhett Boatright</h3>
 */
public class Exercise25_01 {
    ...
    public class BinaryTree<E extends Comparable<E>> extends AbstractTree<E> {
         ...
         */
        public int height() {
            return height(root); //Default using root to find height
        }

        public int height(TreeNode<E> root){
            if(root == null)
                return 0; //If root is empty, return height 0
            int branch1 = height(root.right); //Find height from right branch
            int branch2 = height(root.left); //Find height from left branch
            if(branch1 > branch2)
                return branch1 + 1; //Return height of branch 1 if greater than 2. Add 1 as to not be negative.
            else
                return branch2 + 1; //Return height of branch 2 if branch 1 is not greater. Add 1.
        }
        ...
        public void breadthFirstTraversal() {
            if(this.root == null) return; //End method if root is empty
            // Use this list to compile the traversal
            java.util.LinkedList<TreeNode<E>> queue =
                    new java.util.LinkedList<TreeNode<E>>();
            queue.add(root);
            while(!queue.isEmpty()){ //While the list contains nodes continue cycle

                TreeNode<E> current = queue.element();
                if(current.left != null)
                    queue.add(current.left); //If current left has a child add to the nodeList
                if(current.right != null)
                    queue.add(current.right); //If current right has a child add to the nodeList

                System.out.print(queue.remove().element + " "); //Print out the Breadth
            }
